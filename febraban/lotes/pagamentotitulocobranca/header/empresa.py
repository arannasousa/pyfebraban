...
class Inscricao(TXT):
	def __init__(self, lote=u'0001'):
		...
		self.numero	= TagInteiro(self._tipo_registro, u'10.1', u'numero', 	19, 32, descricao=u'*G006', comentario=u'Número de incricao da empresa', lote=self._lote)
...
class ContaCorrenteAgencia(TXT):
	def __init__(self, lote=u'0001'):
		...
		self.codigo	= TagInteiro(self._tipo_registro, u'12.1', u'codigo', 	53, 57, descricao=u'*G008', comentario=u'Agência mantenedora da conta', lote=self._lote)
		self.dv		= TagCaracter(self._tipo_registro, u'13.1', u'dv', 		58, 58, descricao=u'*G009', comentario=u'Dígito verificador da agência', lote=self._lote)
...
class ContaCorrenteConta(TXT):
	def __init__(self, lote=u'0001'):
		...
		self.numero	= TagInteiro(self._tipo_registro, u'14.1', u'numero', 	59, 70, descricao=u'*G010', comentario=u'Número da conta corrente', lote=self._lote)
		self.dv		= TagCaracter(self._tipo_registro, u'15.1', u'dv', 		71, 71, descricao=u'*G011', comentario=u'Dígito verificador da conta', lote=self._lote)
...
class ContaCorrente(TXT):
	def __init__(self, lote=u'0001'):
		...
		self.dv			= TagCaracter(self._tipo_registro, u'16.1', u'dv', 	72, 72, descricao=u'*G012', comentario=u'Dígito verificador da Ag/Conta', lote=self._lote)
...
class Empresa(TXT):
	def __init__(self, lote=u'0001'):
		...
		self.convenio		= TagCaracter(self._tipo_registro, u'11.1', u'convenio', 33, 52, descricao=u'*G007', comentario=u'Código do convênio no banco', lote=self._lote)
		self.conta_corrente	= ContaCorrente(lote=self._lote)
		self.nome			= TagCaracter(self._tipo_registro, u'17.1', u'nome', 	73, 102, descricao=u'G013', comentario=u'Nome da empresa', lote=self._lote)
