...
class Agencia(TXT):
	def __init__(self, lote=u'0001', num_seq=u'00001', segmento=u'T'):
		...
		self.codigo	= TagInteiro(self._tipo_registro, 	u'08.3T', u'codigo',	 18,  22, descricao=u'*G008', comentario=u'Código da agência mantenedora da conta', segmento=self._segmento, lote=self._lote, num_seq=self._num_seq)
		self.dv	= TagCaracter(self._tipo_registro, 		u'09.3T', u'dv',	 	 23,  23, descricao=u'*G009', comentario=u'Dígito verificador da agência', segmento=self._segmento, lote=self._lote, num_seq=self._num_seq)
