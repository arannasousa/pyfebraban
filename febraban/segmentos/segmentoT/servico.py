...
class Servico(TXT):
	def __init__(self, lote=u'0001', num_seq=u'00001', segmento=u'T'):
		...
		self.numero_registro	= TagInteiro(self._tipo_registro, u'04.3T', u'numero_registro', 	  9,  13, descricao=u'*G038', comentario=u'Número do registro', segmento=self._segmento, lote=self._lote, num_seq=self._num_seq, valor=self._num_seq)
		self.segmento			= TagCaracter(self._tipo_registro, u'05.3T', u'segmento', 			 14,  14, descricao=u'*G039', comentario=u'Tipo do serviço', segmento=self._segmento, lote=self._lote, num_seq=self._num_seq, valor=self._segmento)
		self.CNAB063T 			= TagCaracter(self._tipo_registro, u'06.3T', u'CNAB063T', 			 15,  15, descricao=u'G034', comentario=u'Uso exclusivo FEBRABAN/CNAB', segmento=self._segmento, lote=self._lote, num_seq=self._num_seq)
		self.codigo_movimento	= TagInteiro(self._tipo_registro, u'07.3T', u'codigo_movimento', 	 16,  17, descricao=u'*C004', comentario=u'Código de movimento remessa', segmento=self._segmento, lote=self._lote, num_seq=self._num_seq)
	...
	def get_alertas(self):
		...
		alertas.extend(self.CNAB063T.alertas)
	...
	def get_text(self):
		...
		txt += gera_text(NIVEIS.N4, self.CNAB063T.text)
