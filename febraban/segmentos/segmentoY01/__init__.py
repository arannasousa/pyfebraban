...
class SegmentoY01(TXT):
	def __init__(self, lote=u'0001', num_seq=u'00001', segmento=u'Y', seg_opcional=u'01'):
		...
		self.CNAB183Y				= TagCaracter(self._tipo_registro, 	u'18.3Y', u'CNAB183Y',				156, 240, descricao=u'G004', comentario=u'Uso exclusivo FEBRABAN/CNAB', lote=self._lote, segmento=self._segmento, num_seq=self._num_seq)
	...
	def get_txt(self):
		...
		txt += self.sacador.txt
		txt += self.CNAB183Y.txt
		return txt
	...
	def set_txt(self, arquivo):
		if self._le_txt(arquivo):
			...
			self.sacador.txt = arquivo
			self.CNAB183Y.txt = arquivo
	...
	def get_alertas(self):
		...
		alertas.extend(self.sacador.alertas)
		alertas.extend(self.CNAB183Y.alertas)
		return alertas
	...
	def get_text(self):
		...
		txt += self.sacador.text
		txt += gera_text(NIVEIS.N3, self.CNAB183Y.text)
